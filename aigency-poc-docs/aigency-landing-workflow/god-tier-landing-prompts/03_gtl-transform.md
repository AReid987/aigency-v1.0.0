...
<INSTRUCTION>
	...
	<example-format>
		...
		Visual Style: 
			...
			- Theme: Dark theme with toggle to switch between dark & light mode
			...
			- Error/Alert: {{Semantic Error Color}}

			- Spatial: Layout & Elements all bassed on 8px Grid. Gutters & Margins should be divisible by 8px - 16px, 24px, etc; Size and proximity should be included in creating the visual hierarchyd

			- Borders: {{border width}} {{border color}} on {{elements}}; slightly rounded corners {{border radius}}

			- Typography: {{font family, {{font weight}} for headings, {{font weight}} body, base size 16px

			- Icons/images: {{iconography description}}; {{imagery description}}
...
<Template>
	...
	7. Frequently Asked Questions (Optional): If your consumer offering needs some explanation or tends to get common objections (e.g. “Is my data private?”, “Can I cancel anytime?”), include a brief FAQ. Keep it very concise and reassuring. Common B2C FAQs address things like pricing clarity (“Q: Will I be charged after the free trial? A: No – you can continue on the free plan or choose to upgrade, no automatic charges”), cancellation policies, platform availability (iOS/Android), etc. This section isn’t always necessary for simpler products, but it can remove lingering doubts for more involved services. Importantly, a B2C FAQ should maintain the friendly tone – use simple language, even light humor if it fits your brand, to make the information digestible. The goal is to resolve last-minute hesitations that might block a sign-up.


	8. Final CTA / Sign-Up Section: Close strong with a final CTA section, similar to the B2B template but tailored to individuals. This might be a repeat of your hero tagline or an encouraging statement plus the button. For instance: “Ready to reclaim your weekends from budgeting headaches? Try FinanceGuru today.” followed by the signup button. You could also include a minimal sign-up form here (maybe just an email field + “Get Started” button) if not already provided. Keep it focused: at this stage, the user should have all the info they need, so the final section is purely about converting. No extra links (aside from perhaps Terms in the footer) – you want their full attention on that sign-up. Use a visually clean background and maybe an arrow or directional cue toward the CTA (some landing pages use arrows or even an image of a person looking toward the button – leveraging subtle visual psychology to guide the eye ). Make sure this CTA reflects any offer (if you mentioned a promo, reiterate it: “Start your 14-day free trial (Offer: 2 extra weeks free!)”).



	Tone/Design: Relatable, conversational, and upbeat. B2C copy can be less formal – write as if you’re an encouraging friend or coach helping the user achieve something. Use “you” and “your” frequently, and keep sentences short and lively. Design-wise, you have more freedom to use vibrant colors, illustrative graphics, or playful elements that resonate with your audience’s lifestyle . Just ensure it’s not visually overwhelming – maintain enough whitespace and a logical flow so the eye tracks naturally from one section to the next (remember visual hierarchy: big bold headline, then supporting text, then images, then CTA) . Mobile optimization is critical for B2C, as many consumers will view on phones – use large tap-friendly buttons and legible fonts. Lastly, adapt to industry context: e.g., if your SaaS is in health or finance (where trust is crucial even for consumers), you might incorporate slightly more reassuring language or trust badges; if it’s in entertainment or education, you can be more playful and bold. The template structure remains consistent, but the flavor of language and imagery should align with what appeals in your industry
