...
async def read_user_profile(current_user: TokenData = Depends(get_current_user)):
    """
    Get the profile of the current authenticated user.
    """
    profile = await get_user_profile(current_user.sub)
    if not profile:
...
async def update_user_profile_route(
    ...
    """
    updated_profile = await update_user_profile(current_user.sub, profile_data)
...
async def create_user_profile_route(
    ...
    existing_profile = await get_user_profile(current_user.sub)
    if existing_profile:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Profile already exists for this user.")
    
    created_profile = await create_user_profile(current_user.sub, profile_data)
